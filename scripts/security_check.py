#!/usr/bin/env python3
"""log8-office security preflight checker (non-destructive).
...
def main() -> int:
    print("[security-check] log8-office preflight")

    failures: list[str] = []
    warnings: list[str] = []

    env_mode = (os.getenv("LOG8_OFFICE_ENV") or os.getenv("STAR_OFFICE_ENV") or os.getenv("FLASK_ENV") or "").strip().lower()
    in_prod = env_mode in {"prod", "production"}

    secret = os.getenv("FLASK_SECRET_KEY") or os.getenv("LOG8_OFFICE_SECRET") or os.getenv("STAR_OFFICE_SECRET") or ""
    drawer_pass = os.getenv("ASSET_DRAWER_PASS") or ""

    if in_prod:
        if not is_strong_secret(secret):
            failures.append("Weak/missing FLASK_SECRET_KEY (or LOG8_OFFICE_SECRET) in production")
