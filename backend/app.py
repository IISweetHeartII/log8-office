#!/usr/bin/env python3
"""log8-office - Backend State Service"""
...
app.secret_key = os.getenv("FLASK_SECRET_KEY") or os.getenv("LOG8_OFFICE_SECRET") or os.getenv("STAR_OFFICE_SECRET") or "log8-office-dev-secret-change-me"
...
if is_production_mode():
    ...
    if not is_strong_secret(str(app.secret_key)):
        hardening_errors.append("FLASK_SECRET_KEY / LOG8_OFFICE_SECRET is weak (need >=24 chars, non-default)")
...
# Default state (detail left blank so the UI shows a language-neutral fallback)
DEFAULT_STATE = {
    "state": "idle",
    "detail": "",
    "progress": 0,
...
def join_agent():
    ...
    try:
        ...
        if not isinstance(data, dict) or not data.get("name"):
            return jsonify({"ok": False, "msg": "이름을 입력해 주세요"}), 400
        ...
        if not join_key:
            return jsonify({"ok": False, "msg": "접속 키를 입력해 주세요"}), 400

        keys_data = load_join_keys()
        key_item = next((k for k in keys_data.get("keys", []) if k.get("key") == join_key), None)
        if not key_item:
            return jsonify({"ok": False, "msg": "접속 키가 올바르지 않아요"}), 403
        ...
        with join_lock:
            ...
            if key_expires_at_str:
                try:
                    ...
                    if datetime.now() > key_expires_at:
                        return jsonify({"ok": False, "msg": "이 접속 키는 만료됐어요 (행사 종료) 🎉"}), 403
                except Exception:
                    pass
            ...
            if active_count >= max_concurrent:
                save_agents_state(agents)
                return jsonify({"ok": False, "msg": f"이 접속 키의 동시 접속 한도({max_concurrent})에 도달했어요. 잠시 후 다시 시도하거나 다른 키를 사용하세요"}), 429
        ...
        return jsonify({"ok": True, "agentId": agent_id, "authStatus": "approved", "nextStep": "자동 승인됐어요. 바로 상태 전송을 시작합니다"})
    except Exception as e:
        return jsonify({"ok": False, "msg": str(e)}), 500
...
def leave_agent():
    ...
    try:
        ...
        if not agent_id and not name:
            return jsonify({"ok": False, "msg": "agentId 또는 이름을 입력해 주세요"}), 400
        ...
        if not target:
            return jsonify({"ok": False, "msg": "나갈 agent를 찾지 못했어요"}), 404
...
if __name__ == "__main__":
    ...
    print("=" * 50)
    print("log8-office - Backend State Service")
    print("=" * 50)
    ...
    if is_production_mode():
        ...
        weak_flags = []
        if not is_strong_secret(str(app.secret_key)):
            weak_flags.append("weak FLASK_SECRET_KEY/LOG8_OFFICE_SECRET")
        if not is_strong_drawer_pass(ASSET_DRAWER_PASS_DEFAULT):
