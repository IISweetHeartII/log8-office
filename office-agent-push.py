#!/usr/bin/env python3
"""
log8-office — agent status pusher / 에이전트 상태 푸시 스크립트

An agent runs this to appear in the pixel office and broadcast its work state
(idle / writing / researching / executing / syncing / error). Only the state +
an optional short detail is sent — no task content or private data.

Quick start (env vars, no editing needed):
    OFFICE_URL=http://127.0.0.1:19000 \
    OFFICE_JOIN_KEY=ocj_xxx \
    OFFICE_AGENT_NAME="my-agent" \
    python office-agent-push.py

빠른 시작: 위 3개 환경변수만 주고 실행하면 자동으로 join → 30초마다 상태 푸시.
(환경변수 대신 아래 변수에 직접 채워도 됨.) 상태만 전송, 작업 내용/프라이버시 미전송.
"""
...
# === Config — fill inline OR provide as env vars (env wins when set) ===
JOIN_KEY = os.environ.get("OFFICE_JOIN_KEY", "")       # required: your join key (ocj_xxx)
AGENT_NAME = os.environ.get("OFFICE_AGENT_NAME", "")   # required: your display name in the office
OFFICE_URL = os.environ.get("OFFICE_URL", "http://127.0.0.1:19000")  # office address
...
DEFAULT_STATE_CANDIDATES = [
    os.path.join(OPENCLAW_WORKSPACE_DIR, "log8-office", "state.json"),
...
def do_join(local):
    ...
    if r.status_code in (200, 201):
        ...
        if data.get("ok"):
            ...
            print(f"✅ joined office (agentId={local['agentId']})")
            return True
    print(f"❌ 加入失败：{r.text}")
    return False
